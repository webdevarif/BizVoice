...
// ── scrolling-history waveform (mic bar) ────────────────────────────────────
// Draws `hist` (recent loudness samples, oldest→newest) as rounded bars that
// scroll right-to-left, mirrored around the mid-line. Red to match the app's
// recording accent. Imperative canvas draw — no React re-render per frame.
...
function drawMicHistory(canvas: HTMLCanvasElement, hist: number[]) {
  const ctx = canvas.getContext('2d');
  if (!ctx) return;
  const dpr  = Math.min(window.devicePixelRatio || 1, 2);
  const cssW = canvas.clientWidth  || 54;
  const cssH = canvas.clientHeight || 16;
  if (canvas.width !== Math.round(cssW * dpr) || canvas.height !== Math.round(cssH * dpr)) {
    canvas.width  = Math.round(cssW * dpr);
    canvas.height = Math.round(cssH * dpr);
  }
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  const W = cssW, H = cssH, mid = H / 2, bw = W / HIST_COLS;
  ctx.clearRect(0, 0, W, H);
  for (let i = 0; i < hist.length; i++) {
    const lv = hist[i];
    const h  = Math.max(2, lv * H * 0.95);
    const x  = i * bw;
    ctx.globalAlpha = 0.4 + 0.6 * (i / HIST_COLS); // older (left) columns fade
    const g = ctx.createLinearGradient(0, mid - h / 2, 0, mid + h / 2);
    g.addColorStop(0, '#fca5a5');
    g.addColorStop(1, '#f87171');
    ctx.fillStyle = g;
    roundRectPath(ctx, x + bw * 0.2, mid - h / 2, bw * 0.6, h, Math.min(bw * 0.3, h / 2));
    ctx.fill();
  }
  ctx.globalAlpha = 1;
}
...
export function MicBar() {
  ...
  const startAnalyser = useCallback((stream: MediaStream) => {
    ...
    historyRef.current = new Array(HIST_COLS).fill(0);
    let lastUpdate     = 0;
    ...
    const tick = (now: number) => {
      ...
      if (now - lastUpdate < 55) { vadRafRef.current = requestAnimationFrame(tick); return; }
      lastUpdate = now;
      analyser.getByteFrequencyData(freqData);
      // Overall loudness → push one column, scrolled right-to-left (history).
      let sum = 0;
      for (let k = 0; k < freqData.length; k++) sum += freqData[k];
      const level = Math.min(1, (sum / freqData.length / 255) * 3.2);
      const hist = historyRef.current;
      hist.push(level);
      if (hist.length > HIST_COLS) hist.shift();
      if (canvasRef.current) drawMicHistory(canvasRef.current, hist);
      vadRafRef.current = requestAnimationFrame(tick);
    };
    vadRafRef.current = requestAnimationFrame(tick);
  }, []);
