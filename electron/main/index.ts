...
const TYPE_PS_SCRIPT = [
  ...
  '',
  '    // Chunk size for batched SendInput. One call sends N events atomically',
  '    // — far faster than per-char + Sleep, and reliable in terminals too.',
  '    public const int CHUNK_SIZE = 50;',
  '',
  '    // Single-shortcut paste, app-aware. Uses Shift+Insert for editors and',
  '    // terminals (same approach Wispr Flow uses for Cursor/VS Code/Windsurf',
  '    // integrated terminals) so TUIs like Claude Code/vim accept the paste',
  '    // via bracketed paste mode instead of intercepting Ctrl+V. Falls back',
  '    // to Ctrl+V for normal apps where Shift+Insert is less common.',
  ...
  '        if (proc.StartsWith("code-")) return true;  // VS Code Insiders / Exploration builds',
  '        return false;',
  '    }',
  '',
  ...
  '        return IsTerminal(GetForegroundClassName());',
  '    }',
  '',
  ...
  '    }',
  '',
  '    // Window classes that host Chromium / Gecko-based web content. Any such',
  '    // app may render a TUI inside a webview (Hermes Agent, web Claude Code,',
  '    // Tabby, Wave, custom Electron shells, browsers). Char-typing is the',
  '    // only reliable paste path for these because shortcut events either get',
  '    // forwarded to the page and intercepted by the TUI in raw mode (Ctrl+V),',
  '    // or are not bound to a terminal-pane paste action (Shift+Insert).',
  '    public static bool IsChromiumOrGeckoHost(string cls) {',
  '        if (string.IsNullOrEmpty(cls)) return false;',
  '        return cls == "Chrome_WidgetWin_1"   // Chrome, Edge, all Electron apps',
  ...
  '    }',
  '',
  '    // Fast character typing — builds the full INPUT[] array and sends in',
  '    // chunks via batched SendInput. 100 chars ≈ 30-50ms, instant for voice.',
  '    // Each char becomes 2 KEYEVENTF_UNICODE events (down + up).',
  ...
  '        // Chunked send — 50 events per call keeps terminal input buffers',
  '        // happy while still being ~10× faster than per-char Sleep.',
  ...
  '            #   1. Code editors (Cursor/VS Code/etc.) → Shift+Insert via Paste()',
  '            #      (Shift+Insert pastes via the editor terminal pane → bracketed paste)',
  '            #   2. Native terminals (Windows Terminal/conhost/mintty) → Shift+Insert',
  '            #   3. Any Chromium/Gecko host (browsers, Electron apps that might host',
  '            #      a TUI in a webview — Hermes Agent, web Claude Code, Tabby, etc.)',
  '            #      → char-type. Shortcut events get intercepted by the TUI; raw key',
  '            #      events go through the page DOM cleanly.',
  '            #   4. Native apps → Ctrl+V via Paste()',
  '            $strategy = ""',
